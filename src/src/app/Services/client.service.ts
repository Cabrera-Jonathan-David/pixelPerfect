...
export class ClientService {
  private apiUrlClient = 'http://localhost:5000/clients';

  constructor(private http: HttpClient) {}

  async checkDniExists(dni: string): Promise<boolean> {
    const response = await this.http
      .get<any[]>(`${this.apiUrlClient}?dni=${dni}`)
      .toPromise();
  ...
  async checkUserEmailExists(email: string): Promise<boolean> {
    const response = await this.http
      .get<any[]>(`${this.apiUrlClient}?email=${email}`)
      .toPromise();
    return Array.isArray(response) && response.length > 0; // Devuelve un booleano
  }
  ...
  async getClientById(id: string): Promise<Client | null> {
    const response = await this.http
      .get<Client>(`${this.apiUrlClient}/${id}`)
      .toPromise();
  ...
  async updateClient(id: string, updatedData: Partial<Client>): Promise<void> {
    try {
      await this.http
        .put(`${this.apiUrlClient}/${id}`, updatedData)
        .toPromise();
    } catch (error) {
